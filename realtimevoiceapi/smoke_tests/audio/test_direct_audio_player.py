# here is realtimevoiceapi/smoke_tests/audio/test_direct_audio_player.py


# python -m realtimevoiceapi.smoke_tests.audio.test_direct_audio_player
...
def test_concurrent_playback():
    ...
    # Test 2: Multi-threaded playback
    tests_total += 1
    try:
        player = DirectAudioPlayer()
        results = []
        ...
        def play_tone(freq):
            audio = generate_test_audio(100, frequency_hz=freq)
            return player.play_audio(audio)
        
        # Try to play from multiple threads
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = []
            for freq in [330, 440, 550]:
                future = executor.submit(play_tone, freq)
                futures.append(future)
                time.sleep(0.02)  # Small offset
        ...
        passed = all(results)
        print_result("Multi-threaded playback", passed,
                    f"Threads succeeded: {sum(results)}/3")
        
        if passed:
            tests_passed += 1
    except Exception as e:
        print_result("Multi-threaded playback", False, str(e))
