...
def test_fast_lane_memory_efficiency():
    ...
    try:
        ...
        # Create fast lane components
        components = []
        
        for i in range(10):
            vad = FastVADDetector(config=VADConfig())
            capture = DirectAudioCapture(config=AudioConfig())
            components.append((vad, capture))
        
        # Check object growth
        gc.collect()
        after_creation = len(gc.get_objects())
        growth = after_creation - baseline_objects
        
        print(f"  ✅ Created 10 component pairs")
        print(f"  ✅ Object growth: {growth} objects")
        
        # Process audio through components
        test_audio = np.random.randint(-1000, 1000, 2400, dtype=np.int16).tobytes()
        
        for vad, capture in components:
            for _ in range(100):
                vad.process_chunk(test_audio)
        
        # Check for memory leaks
        gc.collect()
        after_processing = len(gc.get_objects())
        processing_growth = after_processing - after_creation
        
        print(f"  ✅ Processed 1000 chunks total")
        print(f"  ✅ Additional object growth: {processing_growth} objects")
        
        # Should have minimal growth during processing
        assert processing_growth < 100  # Arbitrary but reasonable limit
        ...
        logger.exception("Memory efficiency error")
        return False
